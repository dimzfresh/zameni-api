...
export class PaginationQueryDto {
  ...
  @Min(PAGINATION.MIN_LIMIT, {
    message: `Номер страницы не может быть меньше ${PAGINATION.MIN_LIMIT}`,
  })
  @Max(PAGINATION.MAX_PAGE, {
    message: `Номер страницы не может быть больше ${PAGINATION.MAX_PAGE}`,
  })
  ...
  @Min(PAGINATION.MIN_LIMIT, {
    message: `Количество элементов не может быть меньше ${PAGINATION.MIN_LIMIT}`,
  })
  @Max(PAGINATION.MAX_LIMIT, {
    message: `Количество элементов не может быть больше ${PAGINATION.MAX_LIMIT}`,
  })
  ...
  getPaginationParams(): { page: number; limit: number; offset: number } {
    const page = Math.max(
      ...
      Math.min(PAGINATION.MAX_PAGE, this.page || PAGINATION.DEFAULT_PAGE),
    );
    const limit = Math.max(
      PAGINATION.MIN_LIMIT,
      Math.min(PAGINATION.MAX_LIMIT, this.limit || PAGINATION.DEFAULT_LIMIT),
    );
