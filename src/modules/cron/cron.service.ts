...
export class CronService {
  ...
  async cleanupInactiveUsers() {
    ...
    try {
      ...
      this.logger.log(`Found ${inactiveUsers.length} inactive users to process`);

      // Отправляем каждого пользователя в очередь для асинхронной обработки
      for (const user of inactiveUsers) {
        try {
          await this.queueService.send(QueueTopic.USER_CLEANUP, {
            userId: user.id,
            email: user.email,
          ...
          this.logger.log(`Queued user for cleanup: ${user.email} (ID: ${user.id})`);
        } catch (error) {
          this.logger.error(`Error queuing user ${user.id} for cleanup: ${error.message}`);
        }
      }
  ...
  async notifyInactiveUsers() {
    ...
    try {
      ...
      const soonToBeDeletedUsers = await this.userService.findInactiveUsers(fiveMonthsAgo);
      ...
      for (const user of soonToBeDeletedUsers) {
        try {
          ...
          this.logger.error(`Error sending warning to ${user.email}: ${error.message}`);
        }
      }
