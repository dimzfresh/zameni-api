...
async function bootstrap() {
  ...
  if (rateLimitConfig) {
    ...
    console.log(
      `🛡️ Rate limiting: ${rateLimitConfig.max} requests per ${rateLimitConfig.windowMs}ms`,
    );
  }
  ...
  if (swaggerConfig?.enabled) {
    const config = new DocumentBuilder()
      ...
      .addTag(
        'Администрирование (внутреннее)',
        'Управление пользователями - только для админов',
      )
      .addTag(
        'Очереди (внутреннее)',
        'Управление системой очередей - только для админов',
      )
      .addTag(
        'Автоматическое удаление (внутреннее)',
        'Управление автоматической очисткой - только для админов',
      )
      .addBearerAuth(
        {
          type: 'http',
  ...
  console.log(
    `🚀 Application is running on: http://localhost:${port} (${nodeEnv} mode)`,
  );
